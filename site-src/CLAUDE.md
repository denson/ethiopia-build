...
This directory builds `ethiopia-build.stoagen.com`, the public engineering
record of the Ethiopia knowledge-base build. `public/` is generated output;
never edit it. Edit `site-src/` and run the build.

Voice: plain, exact, unhurried. Never triumphant, never apologetic; a lab
notebook, published. No emoji, no exclamation points, no em-dashes in
reader-facing prose, no marketing. Sentence case everywhere. Numbers are
canonical: every figure comes from the build report and carries a path,
ticket or commit in an evidence strip (a paragraph beginning `Evidence:`).
